...
import { IAddSessionsToAttendeeRequest, IAttendeeGraphModel } from '../../graphql/models/attendee';
import { ISessionGraphModel } from '../../graphql/models/session';
import { ATTENDEE_ADD_SESSIONS } from '../../graphql/mutations/attendee';
...
const AddSessions: React.FC<IAddSessionsProps> = (props) => {
  ...
  const [addSessions] = useMutation<IAttendeeGraphModel>(ATTENDEE_ADD_SESSIONS, {
    onCompleted: (data) => {
      props.onCompleted(data.sessions);
    }
  });
    ...
    await addSessions({
      variables: {
        request: {
          ...
          sessionIds: newSessionIds.map((m) => parseInt(m.toString())),
        } as IAddSessionsToAttendeeRequest,
      },
    });
